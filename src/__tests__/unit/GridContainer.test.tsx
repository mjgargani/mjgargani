...
test('verify if component receives the props and `children` correctly', () => {
  ...
  expect(cardDesc).toHaveStyle(`grid-template-columns: ${props.templateColumns!.mobile!.join(' ')}`);
  expect(cardDesc).toHaveStyle(`grid-template-rows: ${props.templateRows!.mobile!.join(' ')}`);
});
