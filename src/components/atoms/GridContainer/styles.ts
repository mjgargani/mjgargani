...
export const Container = styled.div<GridContainerProps>`
  ...
  grid-template-columns: ${(props) =>
    props.templateColumns?.mobile ? props.templateColumns.mobile.join(' ') : 'repeat(1, 1fr)'};
  grid-template-rows: ${(props) =>
    props.templateRows?.mobile ? props.templateRows.mobile.join(' ') : 'auto'};
  @media ${device.tablet} {
    grid-template-columns: ${(props) =>
      props.templateColumns?.desktop ? props.templateColumns.desktop.join(' ') : 'repeat(1, 1fr)'};
    grid-template-rows: ${(props) =>
